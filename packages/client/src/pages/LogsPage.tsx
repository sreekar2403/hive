...
import { Search, RefreshCw, AlertTriangle, Info, AlertCircle, Filter } from 'lucide-react';
    ...
    <div className="p-6 h-full flex flex-col">
      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        <button onClick={() => setAutoRefresh(!autoRefresh)}
          className={lex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm }>
      ...
      <div className="flex-1 overflow-y-auto bg-gray-950 rounded-xl border border-gray-800">
        {filtered.map((log, i) => {
          ...
          return (
            <div key={log.id} className={lex items-start gap-3 px-4 py-2.5 border-b border-gray-800/50 text-sm }>
              <span className="text-gray-600 font-mono text-xs whitespace-nowrap pt-0.5">{log.timestamp}</span>
              <span className={${bg}  px-1.5 py-0.5 rounded text-xs font-mono flex items-center gap-1}>
