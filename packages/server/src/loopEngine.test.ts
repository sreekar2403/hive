...
describe('LoopEngine', () => {
  ...
  const harnesses = new Map<string, Harness>([['opencode', createMockHarness()]]);
  ...
  describe('retry logic', () => {
    ...
    it('retries on retryable errors', () => {
      ...
      retryableErrors.forEach((stderr) => {
        const shouldRetry = (loopEngine as any).shouldRetry({ success: false, stderr });
        expect(shouldRetry).toBe(true);
      });
    });

    it('does not retry on non-retryable errors when permissions disabled', () => {
      const configNoPermissions = { ...config, permission: { ...config.permission, enabled: false } };
